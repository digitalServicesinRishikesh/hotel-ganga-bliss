...
*/
	...
	// Gallery Popup
	// ----------------------
		$(".wp-block-gallery figure a").each(function(index, el) {
			
			var href = $(this).attr('href');

			if(href.includes(".jpg") || href.includes(".png") || href.includes(".gif")) {

				$(this).closest('figure').addClass("magnificPopup");
			}
		});

		$(".gallery a, .wp-block-gallery figure.magnificPopup a").magnificPopup({
	...
	// Blog Sidebar
	// ----------------------
		if($(".blog-archive .sidebar").length) {

			$(".blog-archive .sidebar .widget").each(function(index, el) {
				
				var widget = $(this);

				if(widget.find('> .wp-widget-group__inner-blocks').length) {

					var widget_wrapper = widget.find('> .wp-widget-group__inner-blocks');

					if(widget_wrapper.find('> .wp-block-categories').length) {
						widget.addClass('widget_categories');
					}
					else if(widget_wrapper.find('> .wp-block-gallery').length) {
						widget.addClass('widget_media_gallery');
					}
					else if(widget_wrapper.find('> .wp-block-archives').length) {
						widget.addClass('widget_categories');
					}
					else if(widget_wrapper.find('> .wp-block-page-list').length) {
						widget.addClass('widget_categories');
					}
				}
			});
	...
	function init() {
		...
		// ----------------------
			if(is_tablet || is_mobile) {
				...
				var right_menu_first = $("header:not(.standard) .menu_location_right_menu nav > ul > li:first-child");

				$("header:not(.standard) .menu_location_left_menu nav > ul > li").each(function(index, el) {
				...
				$("header:not(.standard) .menu_location_left_menu nav").remove();
			}
